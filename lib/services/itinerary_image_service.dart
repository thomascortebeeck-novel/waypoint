...
class ItineraryImageService {
  ...
  Future<ItineraryImages> uploadItineraryImages({
    ...
    try {
      ...
      // Ensure 16:9 aspect for trip cover (plan and overview pages)
      ...
      final base = _ensureAspect(decoded, aspect16x9);
      ...
      final largeUrl = await _storage.uploadImage(path: '$basePath/cover_1024x576.jpg', bytes: largeJpg, contentType: 'image/jpeg');
      final mediumUrl = await _storage.uploadImage(path: '$basePath/cover_512x288.jpg', bytes: mediumJpg, contentType: 'image/jpeg');
      final thumbUrl = await _storage.uploadImage(path: '$basePath/cover_256x144.jpg', bytes: thumbJpg, contentType: 'image/jpeg');
