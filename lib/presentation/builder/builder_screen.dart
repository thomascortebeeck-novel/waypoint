...
class _BuilderScreenState extends State<BuilderScreen> {
    ...
    try {
      ...
      if (plan != null) {
        ...
        if (plan.versions.isNotEmpty) {
          for (final v in plan.versions) {
            final form = _VersionFormData.fromVersion(v);
            // Add listeners to update button state
            form.duration.addListener(() => setState(() {}));
            ...
            // Load FAQ items
            form.faqItems.addAll(
              v.faqItems.map((f) => _FAQFormData.fromModel(f)).toList(),
            );
            _versions.add(form);
          }
        } else {
  ...
  bool _canProceedFromCurrentStep() {
    switch (_currentStep) {
      ...
      case 1: // Step 2: Versions
        ...
        return false;
      case 2: // Step 3: Days
        return true; // No strict validation for step 3
      case 3: // Step 4: Overview
              ...
              ElevatedButton(
                ...
                child: _isSaving
                    ...
                    : Row(
                        ...
                        children: [
                          Text(
                            _currentStep == 3 ? (widget.editPlanId != null ? 'Save Changes' : "Publish Adventure") : "Next",
                            style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14),
                          ),
                          const SizedBox(width: 8),
                          Icon(_currentStep == 3 ? Icons.check : Icons.arrow_forward, size: 18),
                        ],
                      ),
              ),
            ],
  ...
  Widget _buildStepProgressIndicator() {
    final steps = ['General Info', 'Versions', 'Itinerary', 'Overview'];
        ...
        child: ConstrainedBox(
          ...
          OutlinedButton.icon(
            onPressed: () {
              ...
              // Add listeners to update button state
              newVersion.duration.addListener(() => setState(() {}));
              setState(() {
        ...
        children: [
          ...
          Container(
            ...
            child: TabBar(
              ...
              labelStyle: context.textStyles.titleMedium?.copyWith(fontWeight: FontWeight.w600),
              tabs: const [
        ...
        child: ConstrainedBox(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Text(
                  'Build the day-by-day experience for the selected version.',
                  style: context.textStyles.bodyMedium?.copyWith(color: Colors.grey.shade700),
                ),
                const SizedBox(height: 16),
                ...List.generate(
                  vf.daysCount.clamp(0, 60),
                  (i) => Padding(
                    padding: const EdgeInsets.only(bottom: 16),
                    child: _buildDayCard(i + 1, versionIndex: _activeVersionIndex),
                  ),
                ),
                OutlinedButton.icon(
                  onPressed: () {
         ...
         children: [
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 12),
                Expanded(child: _buildTextField("Hiking Time (h)", "0", isNumber: true, controller: timeCtrl, required: false)),
              ],
            ),
             const SizedBox(height: 12),
             Align(
            ...
            // POI WAYPOINTS SECTION (from route)
            if (existingRoute?.poiWaypoints.isNotEmpty ?? false) ...[
              const SizedBox(height: 24),
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text("Points of Interest", style: context.textStyles.titleMedium),
                  Text(
                    '${existingRoute!.poiWaypoints.length} waypoint${existingRoute.poiWaypoints.length != 1 ? 's' : ''}',
                    style: context.textStyles.bodySmall?.copyWith(color: Colors.grey.shade600),
                  ),
                ],
              ),
              ...
              // Display waypoints as compact cards
              ...existingRoute.poiWaypoints.map((wpJson) {
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              // Show accommodation sub-type badge if applicable
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text("Accommodations", style: context.textStyles.titleMedium),
                IconButton(
                  icon: const Icon(Icons.add_circle),
                  tooltip: 'Add Accommodation',
                  onPressed: () => _addWaypointFromItinerary(dayNum, WaypointType.accommodation, vf),
                ),
              ],
            ),
            const SizedBox(height: 8),
            _buildWaypointsListForType(dayNum, WaypointType.accommodation, vf),
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text("Restaurants", style: context.textStyles.titleMedium),
                IconButton(
                  icon: const Icon(Icons.add_circle),
                  tooltip: 'Add Restaurant',
                  onPressed: () {
                    setState(() {
                      if (vf.restaurantsByDay[dayNum] == null) {
                        vf.restaurantsByDay[dayNum] = [];
                      }
                      vf.restaurantsByDay[dayNum]!.add(_RestaurantFormData());
                      _triggerAutoSave();
                    });
                  },
                ),
            ...
            if (restaurants.isEmpty)
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    Text('No restaurants added', style: TextStyle(color: Colors.grey.shade600)),
                  ],
                ),
              )
            else
              ...restaurants.asMap().entries.map((entry) {
                ...
                return Padding(
                  padding: const EdgeInsets.only(bottom: 8),
                  child: _buildRestaurantCard(rest, dayNum, idx, vf),
                );
              }).toList(),
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text("Activities", style: context.textStyles.titleMedium),
                IconButton(
                  icon: const Icon(Icons.add_circle),
                  tooltip: 'Add Activity',
                  onPressed: () => _addWaypointFromItinerary(dayNum, WaypointType.activity, vf),
                ),
              ],
            ),
            const SizedBox(height: 8),
            _buildWaypointsListForType(dayNum, WaypointType.activity, vf),

            // VIEWING POINTS SECTION
            const SizedBox(height: 24),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text("Viewing Points", style: context.textStyles.titleMedium),
                IconButton(
                  icon: const Icon(Icons.add_circle),
                  tooltip: 'Add Viewing Point',
                  onPressed: () => _addWaypointFromItinerary(dayNum, WaypointType.viewingPoint, vf),
                ),
              ],
            ),
            const SizedBox(height: 8),
            _buildWaypointsListForType(dayNum, WaypointType.viewingPoint, vf),
         ],
       ),
     );
  }
  ...
  Future<void> _saveChanges(BuildContext context) async {
    ...
    try {
      ...
      await _planService.updatePlan(updated);
      if (!mounted) return;
  ...
  Future<void> _saveDraft(BuildContext context) async {
    ...
    try {
      ...
      if (widget.editPlanId != null && _editingPlan != null) {
        ...
        final planId = await _planService.createPlan(plan);
        await _userService.addCreatedPlan(userId, planId);
        Log.i('builder', 'New draft created: $planId');
      }
  ...
  /// Triggers auto-save with debouncing (waits 2 seconds after last change)
  void _triggerAutoSave() {
    // Only auto-save when editing existing plans
    if (widget.editPlanId == null) return;
  ...
  /// Performs auto-save without showing snackbar notifications
  Future<void> _autoSaveDraft() async {
    final userId = _auth.currentUserId;
    if (userId == null || _editingPlan == null) return;
    ...
    try {
      ...
      if (_coverImageBytes != null) {
        try {
          ...
          Log.w('builder', 'Failed to upload cover image during auto-save: $e');
        }
      } else if (_heroCtrl.text.trim().isNotEmpty) {
        heroImageUrl = _heroCtrl.text.trim();
      }
      ...
      Log.e('builder', 'Auto-save failed', e, stack);
      if (mounted) {
        setState(() => _saveStatus = 'Failed to save');
      }
    } finally {
  ...
  Widget _buildItineraryCard() {
    ...
    return _SummaryCard(
      ...
      child: hasDays
        ...
        : _EmptyState(message: 'No days added to itinerary', icon: Icons.map_outlined),
    );
  }
  ...
  /// Build list of waypoints for a specific type (for displaying in sections)
  Widget _buildWaypointsListForType(int dayNum, WaypointType type, _VersionFormData vf) {
    final existingRoute = vf.routeByDay[dayNum];
    if (existingRoute == null) {
      return Container(
        padding: const EdgeInsets.all(16),
        ...
        child: Row(
          children: [
            Icon(getWaypointIcon(type), color: Colors.grey.shade600),
            const SizedBox(width: 8),
            Text(
              'Create a route first to add ${getWaypointLabel(type).toLowerCase()}s',
      ...
      return Container(
        ...
        child: Row(
          children: [
            ...
            const SizedBox(width: 8),
            Text(
              'No ${getWaypointLabel(type).toLowerCase()}s added',
              style: TextStyle(color: Colors.grey.shade600),
            ),
          ],
        ),
    ...
    return Column(
      children: waypoints.map((wp) => Padding(
        padding: const EdgeInsets.only(bottom: 8),
        child: Container(
          padding: const EdgeInsets.all(12),
          decoration: BoxDecoration(
          ...
          child: Row(
            children: [
              Icon(getWaypointIcon(wp.type), color: getWaypointColor(wp.type), size: 20),
              const SizedBox(width: 12),
              Expanded(
                child: Text(
                  wp.name,
...
class _VersionFormData {
  ...
  ComfortType comfortType; // Comfort type
  ...
  final List<_FAQFormData> faqItems;
  ...
  _VersionFormData({
    ...
    this.comfortType = ComfortType.none,
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Flexible(child: Text("Experience Level", style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500))),
                    const SizedBox(width: 4),
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                      decoration: BoxDecoration(
                ...
                DropdownButtonFormField<Difficulty>(
                  ...
                  items: Difficulty.values.map((d) {
                    final displayName = d == Difficulty.none 
                        ? 'None' 
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Flexible(child: Text("Comfort Type", style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500))),
                    const SizedBox(width: 4),
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                      decoration: BoxDecoration(
                ...
                const SizedBox(height: 8),
                DropdownButtonFormField<ComfortType>(
                  value: v.comfortType,
                  decoration: InputDecoration(
                    contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 12),
                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)),
                  ),
                  items: ComfortType.values.map((c) {
                    final displayName = c == ComfortType.none 
                        ? 'None' 
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    Text('Add ${getWaypointLabel(widget.type)}', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
                    const Spacer(),
                    IconButton(
