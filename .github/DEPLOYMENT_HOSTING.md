...
- **Google Maps "ApiTargetBlockedMapError" (local or production)**: The key in use does not have "Maps JavaScript API" allowed. **Local:** Copy `.env.example` to `.env` and set `GOOGLE_MAPS_API_KEY=your_key` (`.env` is gitignored). Then run `.\scripts\run_web_local.ps1` (Windows) or `./scripts/run_web_local.sh` (bash)—they load `.env` and pass the key to Flutter. Alternatively create `web/maps_key_local.txt` with the key on one line (gitignored) and use `flutter run -d chrome`. **Production:** Ensure the key in GitHub secret `GOOGLE_MAPS_API_KEY` has "Maps JavaScript API" enabled. In Cloud Console → Credentials → your key → API restrictions, include Maps JavaScript API; under Application restrictions add your domains and `localhost` / `localhost:*` for dev.
