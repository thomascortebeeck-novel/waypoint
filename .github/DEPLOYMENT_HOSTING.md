...
- **Google Maps "ApiTargetBlockedMapError" (local or production)**: The key in use does not have "Maps JavaScript API" allowed. **Local:** Run with a Maps key: `flutter run -d chrome --dart-define=GOOGLE_MAPS_API_KEY=your_key`, or set env `GOOGLE_MAPS_API_KEY` and run `.\scripts\run_web_local.ps1` (Windows) / `./scripts/run_web_local.sh` (bash). **Production:** Ensure the key in GitHub secret `GOOGLE_MAPS_API_KEY` has "Maps JavaScript API" enabled (see "Google Maps API key" above). In Cloud Console → Credentials → your key → API restrictions, include Maps JavaScript API; under Application restrictions add your domains and `localhost` / `localhost:*` for dev.
