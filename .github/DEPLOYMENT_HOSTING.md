...
- **Google Maps "ApiTargetBlockedMapError" on localhost**: The app falls back to the Firebase web API key for the Maps script; that key often has API restrictions that exclude the Maps JavaScript API. To fix: run with a dedicated Maps key that has "Maps JavaScript API" enabled, e.g. `flutter run -d chrome --dart-define=GOOGLE_MAPS_API_KEY=your_maps_api_key`. In [Google Cloud Console](https://console.cloud.google.com/google/maps-apis/credentials), create or use a key with "Maps JavaScript API" (and Places/Geometry if needed) allowed, and under "Application restrictions" add `localhost` and `localhost:*` so it works for local dev.
